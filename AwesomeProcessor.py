...
class AwesomeProcessor:
    ...
    def age_cost(self, df: pd.DataFrame):
        ...
        df['period_age'] = df['period_age'].fillna(0)
        df['period_age'] = df['period_age'].astype(int)
        df['cases'] = df.groupby(['period_age'])['period_age'].transform(lambda x: x.count())
        df['total_cost'] = df['part_cost'] + df['labor_cost']
        df['warranty_end'] = self.add_warranty(df)
        df['expected_lifespan'] = self.add_expected_lifespan(df)

        df['cost_new_asset'] = self.add_cost_new_asset(df)
        df1 = df.groupby(['asset_id', 'producttype_name', 'period_age']).agg(
    ...
    def add_warranty(self, df:pd.DataFrame):
        return pd.to_datetime(df['original_install_date']) + pd.DateOffset(months=12)
    def add_expected_lifespan(self, df:pd.DataFrame):
        pd.to_datetime(df['original_install_date']) + pd.DateOffset(years=21)
    def add_cost_new_asset(self, df:pd.DataFrame):
        max_part_cost = max(df['part_cost'])
        return np.random.randint(max_part_cost * 3, max_part_cost * 4, size=len(df['asset_id']))
